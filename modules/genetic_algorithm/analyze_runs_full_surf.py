...
from modules.genetic_algorithm.plot_ga_results import (plot_best_ind_f_true, 
...
from modules.genetic_algorithm.save_ga_results import write_terms
...
for ind in range(len(best_individuals_sorted)):
    terms = write_terms(best_individuals_sorted[ind], poly_terms, hill_terms)
...
F_true = reaction(u_mesh, v_mesh, true_params)[0]

# Calculate MLP reaction surface
uv = np.column_stack((np.ravel(u_mesh), np.ravel(v_mesh)))
...
if surface_to_fit == 'F_true':
    plot_best_ind_f_true(best_individuals_sorted[0][1:], u_mesh,
                        v_mesh, F_true, poly_terms, hill_terms,
                        'results', zmax=70, ztick=10)
elif surface_to_fit == 'F_mlp':
    plot_best_ind_f_mlp(best_individuals_sorted[0][1:], u_mesh,
                        v_mesh, F_true, F_mlp, poly_terms, hill_terms,
                        'results', zmax=70, ztick=10)
